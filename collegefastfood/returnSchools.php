<?php
/* Reused findNear.php code to return schools from database */
...
if(isset($_POST["lng"]) && isset($_POST["lat"]) && isset($_POST["rad"])){
	if(is_numeric($_POST["lng"]) && is_numeric($_POST["lat"]) && is_numeric($_POST["rad"])){
		...
		$sqlQuery = "select * from emmanuel.school";// where st_distance(location,st_point(".$long.", ".$lat.",1), 'STATUTE MILE') < ".$radius;
...
function populateArray( $sql, $conn, &$dataDump){
	try{
		...
		while($row = db2_fetch_object( $result ) ){
			...
			$rowEntry = array();
			$rowEntry['title1'] = $row->NAME1;
			$rowEntry['title2'] = $row->NAME2;
